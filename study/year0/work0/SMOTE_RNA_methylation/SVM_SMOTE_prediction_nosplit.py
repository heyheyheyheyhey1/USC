...
def train_one_epoch(X, Y, n):
    ...
    for classifier, name in [ (SVM_model, 'SVM'), (RFC_model, 'RF'),
                             (GBM_model, 'GB')]:
...
def oversample(x, y):
    x_os, y_os = SMOTE(random_state=42).fit_resample(x, y)
    x_os, y_os = shuffle(x_os, y_os)
    return x_os, y_os
...
def data_enumerator():
    ...
    postv_num = len(test_positive_data)
    negtv_num = len(test_negative_data)
    for i in range(int(negtv_num / postv_num)):
        neg_batch = test_negative_data[i * postv_num:(i + 1) * postv_num]
        x = np.concatenate([np.array(neg_batch), test_positive_data])
        y0 = np.zeros([len(neg_batch), ])
        y1 = np.ones([postv_num, ])
        y = np.concatenate([y0, y1], axis=0)
        yield x, y, i
def main():
    train_frame = pd.concat([train_negative_frame, train_positive_frame], axis=0)
    train_y = train_frame.iloc[:, -1].values
    train_x = train_frame.iloc[:, 0:-1].values
    x, y = oversample(train_x, train_y)
    train_one_epoch(x, y, "nosplit")
    for name in CLASSIFIER_NAMES:
    ...
    pd_result = pd.DataFrame(columns=['model', 'Accuracy', "Precision", "Recall", "F1"])

    for classifier in CLASSIFIER_NAMES:
        for x, y, i in data_enumerator():
            model = pickle.load(open(os.path.join(MODEL_DIR, classifier, "round_nosplit.sav"), 'rb'))
            pred = model.predict(x)
            scorings = [classifier, accuracy_score(y, pred), precision_score(y, pred),
                        recall_score(y, pred),
                        f1_score(y, pred)]
            pd_result.loc[len(pd_result)] = scorings
    pd_result.to_csv(os.path.join("result","nosplit_test.csv"), index=True, sep="\t")
