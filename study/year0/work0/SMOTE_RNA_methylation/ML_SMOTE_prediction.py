...
# harmonizome_dataset = pd.read_csv(dataset_matrix, delimiter="\t", index_col=0, low_memory=False,nrows=500)
...
random.seed(0)
test_positive_examples = sample(known_rnmts, int(0.2 * len(known_rnmts))) #取正样本20%作为测试

test_rnmts = open(os.path.join(out_dir, "result/test_rnmts.txt"), "w")
for rnmt in test_positive_examples:
    test_rnmts.write(rnmt)
    test_rnmts.write("\n")
test_rnmts.close()

positive_examples = list(set(known_rnmts).difference(set(test_positive_examples))) #用来训练的正样本
...
# All human genes
all_genes = harmonizome_dataset.index.values.tolist() #取矩阵的index 即基因名字
...
random.seed(1)
other_genes = list(set(all_genes).difference(set(known_rnmts))) #负样本
other_genes = shuffle(other_genes)#随机一下

random.seed(2)
test_negative_examples = sample(other_genes, int(0.2 * len(other_genes)))#测试负样本
test_neg = open(os.path.join(out_dir, "result/test_negative.txt"), "w")
for negative in test_negative_examples:
    test_neg.write(negative)
    test_neg.write("\n")
test_neg.close()

negative_genes = list(set(other_genes).difference(set(test_negative_examples))) #训练负样本
...
positive_examples_in_all_genes = list(set(all_genes).intersection(set(positive_examples))) #取已知正样本与全部基因的交集
...
def SVM_tuning(n, n_dir, X_train, y_train):
    ...
    for score in scores:
        svm_tuning = GridSearchCV(SVC(random_state=3), grid_param_svm, cv=KFold(3, shuffle=True, random_state=3), scoring='%s' % score, n_jobs=12)
...
def RF_tuning(n, n_dir, X_train, y_train):
    ...
    for score in scores:
        rf_tuning = GridSearchCV(RandomForestClassifier(random_state=3), grid_param_rf, cv=KFold(3, shuffle=True, random_state=3), scoring='%s' % score, n_jobs=12)
    ...
    for score in scores:
        rf_random = RandomizedSearchCV(estimator=RandomForestClassifier(n_estimators = rf_tuning.best_params_['n_estimators'], random_state=3), param_distributions=grid_param_rf_random, n_iter=100, cv=KFold(3, shuffle=True, random_state=3), verbose=0, scoring='%s' % score, n_jobs=12)
...
def GM_tuning(n, n_dir, X_train, y_train):
    ...
    for score in scores:
        gbm_tuning = GridSearchCV(GradientBoostingClassifier(subsample=0.8, random_state=3), grid_param_gbm, cv=KFold(3, shuffle=True, random_state=3), scoring='%s' % score, n_jobs=12)
    ...
    for score in scores:
        gbm_random = RandomizedSearchCV(estimator=GradientBoostingClassifier(n_estimators = gbm_tuning.best_params_['n_estimators'], subsample=0.8, random_state=3), param_distributions=grid_param_gmb_random, n_iter=100, cv=KFold(3, shuffle=True, random_state=3), verbose=0, scoring='%s' % score, n_jobs=12)
...
def model_training_n(n=0):
    ...
    for classifier, name in [(LR_model, 'LR'),
                             ...
                             (RFC_model, 'RF'),
                             (GBM_model, 'GB')]:
        scores = cross_validate(estimator=classifier, X=X_train, y=y_train, cv=10, scoring=scorings, n_jobs=12)
