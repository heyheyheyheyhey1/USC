...
selected_dataset = "selected_dataset.tsv"
dataset_matrix = os.path.join("data", selected_dataset)
selected_dataset = pd.read_csv(dataset_matrix, delimiter="\t", index_col=0, low_memory=False)
features_to_keep = selected_dataset.columns
...
# All human genes
all_genes = selected_dataset.index.values.tolist()  # 取矩阵的index 即基因名字
...
epoch = int((len(negative_genes) / num_positive_examples * 0.6))
...
def negative_sample_draw(gene_list, n=0):
    """get the nth chunck of negative examples"""
    l = int(len(negative_genes) / epoch)
...
def model_training_n(n=0):
    ...
    X_train, y_train = SMOTEN(random_state=42).fit_resample(X_train, y_train)
