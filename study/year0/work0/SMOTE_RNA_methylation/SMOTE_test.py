...
# selected dataset
selected_dataset = pd.read_csv(os.path.join(out_dir, "selected_dataset.tsv"), delimiter="\t", index_col=0,
...
def test_set_n(n=0):
    ...
    # Double-check that the test dataset does not contain labels 检查属性中不含targets标签
    test_data = test_dataset_n.iloc[:, 0:-1]  # 去掉最后一个标识位Targets
    for i in range(len(test_data.columns)):
        data = abs(test_data.iloc[:, i])
        if data.equals(test_dataset_n.iloc[:, -1]):
            raise Exception("Chunk n:", n, "target labels match feature:", i, test_data.columns[i], "nFeatures: ",
                            test_data.shape[1])
...
def model_performance_test(m="GB", r=2, n=0):  # r代表训练出的第r个模型 n代表第n份测试数据
    ...
    n_predictions = {}
    model_run = namedtuple("model_run", ["model", "sample"])  # 定义一个model_run类 包含model和sample两个属性

    # Evaluate range r of models m
    for i in range(r):
        # load model
        read_dir = os.path.join(out_dir, f'draw_{i}')
        model = pickle.load(open(os.path.join(read_dir, f'{m}_m{i}.sav'), 'rb'))  # 读取GB模型第i次训练
        ...
        n_predictions[model_run(m, i)] = test_predictions  # 记录所有的结果
...
for t in range(299):
    model_performance_test(m="GB", r=174, n=t)
