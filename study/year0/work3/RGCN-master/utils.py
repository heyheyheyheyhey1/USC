...
def load_data(file_path):
    ...
    return entity2id, relation2id, train_triplets, test_triplets
...
def edge_normalization(edge_type, edge_index, num_entity, num_relation):
    '''
        ...
        - edge_norm: (num_edge)
    '''
    one_hot = F.one_hot(edge_type.to(torch.int64), num_classes = 2 * num_relation).to(torch.int64)
    deg = scatter_add(one_hot, edge_index[0].to(torch.int64), dim = 0, dim_size = num_entity)
    index = edge_type + torch.arange(len(edge_index[0])) * (2 * num_relation)
    edge_norm = 1 / deg[edge_index[0].to(torch.long)].view(-1)[index]
